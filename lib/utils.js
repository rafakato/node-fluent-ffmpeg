...
var utils = module.exports = {
   ...
   * @private
   */
  extractCodecData: function(command, stderrLine, codecObject) {
    var format= /Input #[0-9]+, ([^ ]+),/.exec(stderrLine);
    var dur   = /Duration\: ([^,]+)/.exec(stderrLine);
    var audio = /Audio\: (.*)/.exec(stderrLine);
    var video = /Video\: (.*)/.exec(stderrLine);

    if (!('format' in codecObject)) {
      codecObject.format = '';
      codecObject.audio = '';
      codecObject.video = '';
      codecObject.duration = '';
    }
    ...
    if (streamInputs.length === 0) {
      codecInfoPassed = /Press (\[q\]|ctrl-c) to stop/.test(stderrLine);
    } else {
      codecInfoPassed = /^Output #/.test(stderrLine);
    }
